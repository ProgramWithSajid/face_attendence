...
def scan(request):
    ...
    for profile in profiles:
        ...
        person_face_encoding = face_recognition.face_encodings(image_of_person)[0]  #[0]
...
def loginadmin(request):
    if request.method=="POST":
        username = request.POST.get('username')                     #taking username & pass
        password = request.POST.get('password')
        print(username,password)
        #check if user has entered correct credientials
        user = authenticate(username=username,password=password)     #authenticate check if user & pass matches
        if user is not None:
